<template>
  <div class="vue-infinite-grid"></div>
</template>
...
export default {
  ...
  methods: {
    ...
    getItem(options) {
      let imageName =
        options.width > 620 && options.height > 620
          ? options.name + "_thumb.png"
          : options.name + "." + options.ext;
      let _path =
        "file://" +
        path.join(
      ...
      return `<div class="item" id="item-${options.id}">
          <div class="thumbnail ">
            <img class="thumbnial-img tbackground" draggable="true" src="${_path}" data-src="${_path}" data-width="${
        options.width
      }" data-height="${options.height}">
          </div>
          <!-- <div class="tag-num">${parseInt(
            Math.random() * 10 + 1
          )}</div> -->
  ...
  computed: {
    itemSize() {
      return (store.state.App.zoomLevel + 1) * 75;
    },
    selected: {
      get() {
